...
function possibleIntersect(min1, max1, min2, max2) {
    return !(max1.x < min2.x || max2.x < min1.x ||
        max1.y < min2.y || max2.y < min1.y);
}

function polyInterset(poly1, poly2) {
    ...
    for (var i1 = 0; i1 < l1 ; i1++) {
        for (var i2 = 0; i2 < l2; i2++) {
            if(possibleIntersect(poly1[i1],poly1[i1+1],poly2[i2],poly2[i2+1])){
                if(isIntersect(poly1[i1],poly1[i1+1],poly2[i2],poly2[i2+1])){
                    return 1;
                }
            }
        }
